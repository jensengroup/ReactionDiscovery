...
def write_meta_input(time, kpush, alp, scale_factor):
...
def setup_restart(time_ps, n_atoms, current_time):
    """
    ...
    for i in range(int(time_ps)-1):
        ...
        with open('initial_structures.xyz', 'a') as ofile:
    ...
    if not RESTARTED:
        ...
        contents.insert(10, "   coord=initial_structures.xyz\n")
...
if __name__ == "__main__":
    ...
    while AC_SAME and CURRENT_TIME < MAX_TIME:
        do_metadynamics_calculation(MD_FILE, TIME, K_PUSH, ALP, SCALE_FACTOR,
                                    CHARGE, CURRENT_TIME)
        setup_restart(TIME, N_ATOMS, CURRENT_TIME)
